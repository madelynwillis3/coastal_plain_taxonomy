...
# -----------------------------------------------------------------------------
OUTCOMES_TO_TEST <- c("taxgrtgroup", "taxsubgrp_mod1")  # add others if time allows
...
quick_check <- part1 %>%
  ...
  group_by(outcome, engine) %>%
  summarise(
    default_ncss = ncss_kap[n_neighbors == 15 & min_dist == 0.1 & minPts == 10],
    best_ncss    = max(ncss_kap, na.rm = TRUE),
    ncss_improvement = best_ncss - default_ncss,
    .groups = "drop"
  )
print(quick_check)
```
